...
locals {
  ...
  mysql_conn = local.azure_enabled && local.from_secret_enabled ? {
    ...
    tls         = try(var.azure.sslmode, "true")
    } : local.azure_enabled ? {
    host        = data.azurerm_mysql_flexible_server.this[0].fqdn
    port        = 3306
    username    = try(var.azure.admin_username, "adminuser")
    password    = try(var.azure.admin_password, "")
    db_name     = try(var.azure.db_name, "mysql")
    server_name = try(var.azure.server_name, local.system_name)
    tls         = try(var.azure.sslmode, "true")
    } : local.hoop_connect ? {
    host        = "127.0.0.1"
    port        = try(var.hoop.port, 3307)
    username    = try(var.hoop.username, "noop")
    password    = try(var.hoop.password, "noop")
    db_name     = try(var.hoop.db_name, "mysql")
    ...
    db_name     = try(var.direct.db_name, "mysql")
    server_name = try(var.direct.server_name, "")
    tls         = try(var.direct.tls, "true")
  }
}
